...
class AnomalyExplainer:
    def __init__(self, model):
        self.model = model
        self.explainer = None
        self.feature_names = ["cpu_raw", "cpu_mean_5", "cpu_std_5", "mem_raw", "is_wsl", "is_container", "is_host"]
    ...
    def explain(self, feature_vector: list) -> dict:
        ...
        try:
            ...
            sorted_impact = sorted(importance.items(), key=lambda x: abs(x[1]), reverse=True)
            return {
                "top_contributors": [{"feature": k, "impact": round(v, 4)} for k, v in sorted_impact[:3]]
            }
        except Exception as e:
            return {"error": str(e)}
